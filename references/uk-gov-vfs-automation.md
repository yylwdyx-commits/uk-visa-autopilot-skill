...
- **护照不押中心，录完指纹当场归还**（eVisa 时代不贴签证页，VFS 只扫描）。VFS 当晚那封「The passport … was collected from the Visa Application Centre」= 申请人本人取走；e-ICR 回执上的「Passport collection timing」是通用模板句，别据此推「押本」。★我曾读反成「快递收走送审、出签后去取」，让人白等一周——别再犯。★实测：录指纹→出签 **5 个工作日**（官方口径 3 周是上限，别按 3 周排行程）。
...
- 2026-02-25 起访问签只发 **eVisa**，护照上不贴签证页。出签后两步（护照本来就在手上，不存在取件）：①本人手机装「UK Immigration: ID Check」扫护照芯片+刷脸建 **UKVI 账户**（gov.uk/evisa/set-up-ukvi-account，免费，≠申请时的 gov.uk 登录账号；VFS 现场推销的「查签证账户」就是这个，别买）；②登进 view-evisa 核对姓名/护照号/类型 VISIT/有效期，错了走 gov.uk/evisa/report-error-evisa 报错。
- ★建账户 + 链接 eVisa 的**逐屏答案和三个坑**（短信码收不到怎么跳过、ID Check 应用报错怎么办、一台手机怎么做完）见 `ukvi-account-evisa.md`，照那个给申请人发操作示意，别按概述页给大纲。
